...
def checksum(s: 'iterable', calc: 'function', Key: 'iterable',
             Sum: list) -> chr:
    '计算校验位'
    return Sum[sum(calc(c, k) for c, k in zip(s, Key)) % len(Sum)]


def id_card(card_no: str) -> str:
    '''修正居民身份证的校验位'''
    ...
    if 15 == length:
        ...
        length += 2
    if 18 >= length >= 17:
        return card_no[:17] + checksum(card_no[:17], lambda x, y: int(x) * y,
                                       Key, Sum)
...
def bank_card(card_no: str) -> str:
    '''修正银行卡校验位'''
    Sum = '0987654321'
    if len(card_no) in (16, 17, 19):
        card_no = card_no[:-1]
        return card_no + checksum(
            reversed(card_no), lambda x, y: sum(divmod(int(x) * y, 10)),
            cycle([2, 1]), Sum)
